...
spherical-harmonic, or custom manifolds with explicit local frames. For an
...
**Hoppings** selects compact Slater--Koster integrals or the complete general
symmetry-matrix basis, then uses a distance cutoff to generate suggestions for
...
Hamiltonian once and then reuse it. GUI-built three-dimensional models also
fold centered conventional cells to the primitive translation cell by default.
...
project widgets. CIF-installed models default to the Hinuma/HPKOT
high-symmetry path generated by Seek-path in the primitive reciprocal basis.
Manual reduced coordinates remain available; `[0.5, 0, 0]` means
$\mathbf b_1/2$ and is not a universal definition of X for every Bravais
lattice.
