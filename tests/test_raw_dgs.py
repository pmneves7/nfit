...
from nfit.raw_dgs import (
    TOF_US_PER_M_SQRT_MEV, _evaluate_mantid_t0_formula, _monitor_ei_t0,
...
def test_raw_dgs_applies_mantid_he3_tube_efficiency(monkeypatch, tmp_path):
    ...
    exponent = raw_dgs.HE3_EFFICIENCY_EXPONENTIAL_CONSTANT * (10.0 / 290.0) * (2.0 * (0.01 - 0.0008))
...
def test_raw_dgs_feldman_cousins_zero_error_is_normalized(monkeypatch, tmp_path):
    ...
    result = bin_raw_dgs_group(
        group, lower=[-10, -10, -10, 19], upper=[10, 10, 10, 20], num_bins=[1, 1, 1, 1],
    )
...
def test_monitor_fit_discovers_non_sequoia_monitor_names(tmp_path):
    ...
    with h5py.File(source, "w") as handle:
        ...
        for name, distance in zip(("upstream_monitor", "downstream_monitor"), distances):
            centre = t0 + TOF_US_PER_M_SQRT_MEV * distance / np.sqrt(energy)
            entry.create_group(name).create_dataset("event_time_offset", data=np.repeat(centre, 200))
    with h5py.File(source, "r") as handle:
...
def test_monitor_fit_uses_idf_order_and_log_driven_positions(tmp_path):
    ...
    with h5py.File(source, "w") as handle:
        ...
        entry.create_group("DASlogs").create_group("msd").create_dataset("average_value", data=[msd])
        for name, distance in zip(("monitor1", "monitor2"), distances):
            centre = t0 + TOF_US_PER_M_SQRT_MEV * distance / np.sqrt(energy)
            monitor = entry.create_group(name); monitor.attrs["NX_class"] = "NXmonitor"
...
def test_mantid_cncs_t0_formula_uses_requested_incident_energy():
    ...
    expected = 288.595706061 - 110.833514059 / np.sqrt(energy) + 89.6080314589 / energy - 42.6999684563 * np.sqrt(energy) + 1.89672170078 * energy
