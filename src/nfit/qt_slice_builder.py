...
def _build_axis_controls(viewer: Any, controls_layout: Any) -> None:
    ...
    viewer.swap_axes_button = QtWidgets.QPushButton("Swap x/y")
    viewer.swap_axes_button.setToolTip("Exchange the horizontal and vertical displayed axes.")
    viewer.swap_axes_button.clicked.connect(viewer._swap_display_axes)
    axis_selector_layout.addWidget(viewer.swap_axes_button)
    axis_selector_layout.addStretch(1)
    axes_layout.addWidget(axis_selector_widget, 0, 0, 1, 5)
    viewer.axes_ratio_combo = QtWidgets.QComboBox()
    viewer.axes_ratio_combo.addItem("Fit panel", "fit")
    viewer.axes_ratio_combo.addItem("Equal inverse Å", "q")
    viewer.axes_ratio_combo.addItem("Equal r.l.u.", "rlu")
    viewer.axes_ratio_combo.setToolTip(
        "Set the 2D axes ratio: fit the panel, preserve reciprocal-space length "
        "along each displayed momentum axis, or preserve r.l.u. distance along those axes."
    )
    viewer.axes_ratio_combo.setProperty("default_tooltip", viewer.axes_ratio_combo.toolTip())
    viewer.axes_ratio_combo.currentIndexChanged.connect(viewer._set_axes_ratio)
    viewer.axes_ratio_label = QtWidgets.QLabel("Axes ratio")
    axes_layout.addWidget(viewer.axes_ratio_label, 4, 0)
    axes_layout.addWidget(viewer.axes_ratio_combo, 4, 1, 1, 4)
